"""The file handles the extraction of protein adjacency and feature data from BGF files.
"""
...
class BGFDataFile:
    """The class uses a BGF and PDF protein file to generate protein feature and
        adjacency matrices.
    """
    ...
    def getAdjacencyMatrix(self, target_folder=config.PROTEIN_ADJACENCY_PATH):
        """The function creates the adjacency matrix for a given protein and
            stores in a target folder.

        Args:
            target_folder (str): Path to the folder storing the adjacency protein matrices.
                Defaults to config.PROTEIN_ADJACENCY_PATH.
        """

        log.info('Initiated creation of BGF Adjacency matrix for protein %s', self.protein_name)
        adjacency_matrix = np.zeros((self.atom_count, self.atom_count))
        with open(self.bgf_filename, 'r', encoding='utf-8') as bgf_file:
    ...
    def getFeatureMatrix(self, target_folder=config.PROTEIN_FEATURE_PATH):
        """The function creates the feature matrix for a given protein and
            stores in a target folder.

        Args:
            target_folder (str): Path to the folder storing the feature protein matrices.
                Defaults to config.PROTEIN_FEATURE_PATH.
        """

        log.info('Initiated creation of BGF Feature matrix for protein %s', self.protein_name)
        ...
        for atom in molecule:
            ...
            feature_matrix[atom_index][-2] = self.__normalize(atom_types[atom_index], 4.0)
            feature_matrix[atom_index][-1] = self.__normalize(confidence_scores[atom_index], 100.0)
            atom_index += 1
    ...
    def __getMoleculeDataRow(self, atom):
        ...
        features_arr[0] = self.__normalize(atom.atomicmass, 32.065)
        features_arr[1] = self.__normalize(atom.exactmass, 31.972071)
        features_arr[2] = self.__normalize(atom.heavydegree, 4.0)
        features_arr[3] = self.__normalize(atom.heterodegree, 3.0)
        features_arr[4] = self.__normalize(atom.hyb, 3.0)
        # TODO: create a classification index which is based on where in the
        # protein can we find the atom
        features_arr[5] = self.__normalize(atom.idx, 3500.0)
        features_arr[6] = self.__normalize(atom.partialcharge, 0.6, -0.6)
        features_arr[7] = self.__normalize(atom.degree, 4.0)
        return features_arr
    ...
    def __getConfidenceScores(self):
        log.info('Initiated extracting of confidence scores from PDB file for %s',
                self.protein_name)
        confidence_scores = np.zeros(self.atom_count, dtype='float')
        with open(self.pdb_filename, encoding='utf-8') as pdb_file:
    ...
    def __getAtomTypes(self):
        log.info('Initiated extracting of atom types from PDB file for %s', self.protein_name)
    ...
    def __normalize(self, old, maximal, minimal=0.0):
        new = (float(old) - float(minimal))/(float(maximal) - float(minimal))
        return new
